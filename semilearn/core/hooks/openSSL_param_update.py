...
class openSSL_ParamUpdateHook(Hook):
    """
    ...
    def after_train_step(self, algorithm):
        loss_c = algorithm.out_dict['loss_c']
        loss_s = algorithm.out_dict['loss_s']

        # update parameters
        if algorithm.use_amp:
            raise NotImplementedError
        else:
            algorithm.model['model_c'].zero_grad()
            loss_c.backward()
            algorithm.optimizer['optimizer_c'].step()

            algorithm.model['model_s'].zero_grad()
            algorithm.model['decoder'].zero_grad()
            loss_s.backward()
            algorithm.optimizer['optimizer_s'].step()
            algorithm.optimizer['optimizer_decoder'].step()
