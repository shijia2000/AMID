    ...
    def after_train_step(self, algorithm):
        ...
        if self.every_n_iters(algorithm, algorithm.num_eval_iter):
            if not algorithm.distributed or (algorithm.distributed and algorithm.rank % algorithm.ngpus_per_node == 0):
                ...
                print_text += "BEST_OUT_ACC: {:.4f}, at {:d} iters, ".format(algorithm.best_out_acc, algorithm.best_out_it + 1)
                print_text += "BEST_ALL_ACC: {:.4f}, at {:d} iters".format(algorithm.best_all_acc, algorithm.best_all_it + 1)
