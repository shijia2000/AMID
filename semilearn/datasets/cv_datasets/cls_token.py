...
def cal_cls_token(args, dataset):
    cls_token_path = os.path.join(args.cluster_dir, 'cls_token.pkl')
    os.makedirs(args.cluster_dir, exist_ok=True)
    if os.path.exists(cls_token_path):
        with open(cls_token_path, 'rb') as file:
            cls_token_list = pickle.load(file)
        return cls_token_list
    # if os.path.exists(os.path.join(args.cluster_dir, 'cluster_feat.bin')):
    #     cls_token = np.load(os.path.join(args.cluster_dir, 'cluster_feat.bin'))
    ...
    extractor = VitExtractor(model_name='dino_vitb16', device='cuda')
    texture_transforms = dino_texture_transforms
    base_transform = transforms.Compose([transforms.ToTensor(), ])
    global_B_patches = transforms.Compose([texture_transforms, Global_crops(n_crops=1, min_cover=0.95, last_transform=base_transform)])
    imagenet_norm = transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))
    global_resize_transform = transforms.Resize(224)
    global_transform = transforms.Compose([global_resize_transform, imagenet_norm])

    cls_token_list = []
    for img_path in dataset.data:
        img = Image.open(img_path).convert('RGB')
        img = global_B_patches(img)
        img = img.squeeze()
        img = global_transform(img).unsqueeze(0).to(device)

        with torch.no_grad():
            cls_token = extractor.get_feature_from_input(img)[-1][0, 0, :]
        cls_token_list.append(cls_token.cpu().numpy())
    with open(cls_token_path, 'wb') as file:
        pickle.dump(cls_token_list, file)
    return cls_token_list
...
class VitExtractor:
    ...
    def __init__(self, model_name, device):
        ...
        # self.model = torch.hub.load('/opt/data/private/shijia/SSL_DG/pretrained_model/facebookresearch_dino_main', model_name,trust_repo=True,source='local').to(device)
